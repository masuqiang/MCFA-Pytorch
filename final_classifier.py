...
class CLASSIFIER:
    def __init__(self,model, _train_X, _train_Y,_test_seen_X,_test_seen_Y,_test_novel_X, _test_novel_Y, seenclasses,novelclasses,
        ...
        self.train_X =  _train_X.to(self.device)#这里边包含了所有用于训练的可见类和不可见类的视觉特征、语义特征
        self.train_Y = _train_Y.to(self.device)#这里的X里边的视觉特征和语义特征分别对应的标签

        self.test_seen_feature = _test_seen_X.to(self.device)#这个应该是在广义零样本的时候用到
        self.test_seen_label = _test_seen_Y.to(self.device)
        self.test_novel_feature = _test_novel_X.to(self.device)#零样本的时候，应该只用到这个即可
    ...
    def val(self, test_X, test_label, target_classes):
        ...
        for i in range(0, ntest, self.batch_size):
            ...
            _, predicted_label[start:end] = torch.max(output.data, 1)
            #print("predicted_label[start:end]: ", predicted_label[start:end])
            start = end
